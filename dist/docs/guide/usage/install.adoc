...
Prospero provides the possibility to install using available Maven artifacts present on https://mvnrepository.com/artifact/org.wildfly.channel[WildFly Channel page], by using `groupID:artifactID` on the `--manifest` parameter.
...
To install a specific version of WildFly, use the `--manifest` parameter with the `groupID:artifactID:version` format. This is useful when you need to install a particular release for testing or compatibility reasons.

[source, bash]
----
$ ./prospero.sh install --dir wf-38 --profile wildfly-ee --manifest org.wildfly.channels:wildfly-ee:38.0.0.Final
----

NOTE: You can check available versions on https://mvnrepository.com/artifact/org.wildfly.channel[Maven Repository].
